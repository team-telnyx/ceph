...
std::vector<ceph::libfdb::select> locate_split_points(ceph::libfdb::database_handle dbh, ceph::libfdb::select selector, const std::int64_t remote_chunk_size)
{
 using ceph::libfdb::detail::await_future_of;

 auto txn = ceph::libfdb::make_transaction(dbh);

 // We can do this without side-effects by combining some lambdas, but for now I'm satisfied if it works-- architecturally, this should be
 // implemented in Transaction or Database and exposed via friends, like the rest of the library;
