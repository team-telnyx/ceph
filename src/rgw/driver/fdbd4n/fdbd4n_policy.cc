...
#if 0
    if (victim->cacheObj.hostsList.size() == 1 && *(victim->cacheObj.hostsList.begin()) == dpp->get_cct()->_conf->rgw_fdbd4n_local_rgw_address) { /* Last copy */
...
void LFUDAPolicy::cleaning(const DoutPrefixProvider* dpp)
{
  ...
  while(!quit) {
	...
	if (ret < 0) {
	  ...
	  //Remove bucket should be implemented in fdbd4n which will take care of deleting objects belonging to the bucket, and hence we should not reach here
	  erase_dirty_object(dpp, e->key, null_yield);
	  continue;
	}
	...
	if (!c_obj->have_instance()) {
	  ...
	  if (op_ret < 0) {
	    ...
	    if (block.version == e->version) {
	      rgw::fdbd4n::CacheBlock null_block;
	      null_block = block;
	  ...
	  ldpp_dout(dpp, 10) << "FDBD4NFilterObject::" << __func__ << "(): Removing object name: "<< c_obj->get_name() << " score: " << std::setprecision(std::numeric_limits<double>::max_digits10) << e->creationTime << " from ordered set" << dendl;
	...
	if (c_obj->have_instance()) { //versioned case
	  ...
	  rgw::fdbd4n::CacheBlock instance_block;
