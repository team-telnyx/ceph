...
/* JFW: we should probably move this somewhere useful, I'm sticking it in here so we have a way to log things during porting: */
inline void dout_trace(const std::string_view info, const std::string_view msg_tag = "fdbd4n-trace", const std::source_location sl = std::source_location::current())
{
...
{
  fdbd4n_writecache = g_conf()->d4n_writecache_enabled;
...
{
  dout_trace("JFW: calling newFDBD4NFilter()");
  auto *driver = new rgw::sal::FDBD4NFilterDriver(next, *static_cast<boost::asio::io_context *>(io_context), admin);
  dout_trace("JFW: ok, FDB D4N FilterDriver established.");
